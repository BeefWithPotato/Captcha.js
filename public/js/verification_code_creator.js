...
TextEnterGenerator.prototype = {
    ...
    addDots: function(num, radius, speed){
    	let i;
    	for(i = 0; i < num; i++){
    		...
    		newDot.radius = radius;
    		newDot.speedX = this.getRandomSpeed(speed);
    		newDot.speedY = this.getRandomSpeed(speed);
    ...
    moveDots: function(){
    	...
    	for(i = 0; i < this.dots.length; i++){
    		...
    		if(this.dots[i].speedX === 0 && this.dots[i].speedY === 0){
    			this.dots[i].speedX = this.getRandomSpeed(this.dotSpeed);
    			this.dots[i].speedY = this.getRandomSpeed(this.dotSpeed);
    		}
    ...
    drawRandomDotsWithAnimation: function(canvas){
    	...
    	this.moveDots();
    	canvas.height = canvas.height;
    	this.drawRandomDots(canvas, this.dots.length, 2);
    ...
    drawRandomLines: function(canvas, num, width){
    	const canvasContext = canvas.getContext("2d");
    	for(let i = 0; i < num; i++){
